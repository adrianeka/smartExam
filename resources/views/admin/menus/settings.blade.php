                ...
                @if($tab === 'overview')
                    ...
                    <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            @if($menu->type !== 'category')
                            <div>
                                <label for="icon" class="block text-sm font-semibold text-gray-700 mb-2">Icon (FontAwesome Class)</label>
                                <input type="text" name="icon" id="icon" value="{{ old('icon', $menu->icon) }}" class="block w-full px-4 py-3 bg-gray-100 border-transparent focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-lg transition-colors text-gray-800 font-medium">
                                <p class="text-xs text-gray-500 mt-2">Example: <code class="bg-gray-200 px-1 rounded">fa-solid fa-book</code></p>
                            </div>
                ...
                @elseif($tab === 'permissions' && $menu->type !== 'category')
                    ...
                    >
                        ...
                        <template x-for="role in roles" :key="'role-'+role.id">
                            <div>
                                <input type="hidden" :name="'roles[' + role.id + '][view]'" value="1" x-bind:disabled="!role.view">
                                <input type="hidden" :name="'roles[' + role.id + '][edit]'" value="1" x-bind:disabled="!role.edit">
                                <template x-for="user in role.users" :key="'user-'+user.id">
                                    <div>
                                        <input type="hidden" :name="'users[' + user.id + '][view]'" value="1" x-bind:disabled="!user.view">
                                        <input type="hidden" :name="'users[' + user.id + '][edit]'" value="1" x-bind:disabled="!user.edit">
                                    </div>
                                </template>
                            </div>
                        </template>
