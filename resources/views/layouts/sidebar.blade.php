<aside id="sidebar"
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden py-4 px-3">
        ...
        {{-- Manajemen Pengguna --}}
        <div x-data="{ open: {{ request()->routeIs('admin.users.*') || request()->routeIs('user.*') ? 'true' : 'false' }} }" class="mb-0.5">
            <button @click="open = !open"
                    class="nav-item w-full flex items-center justify-between px-2 py-2.5 rounded-lg {{ request()->routeIs('admin.users.*') || request()->routeIs('user.*') ? 'text-blue-600 bg-blue-50/60 font-semibold' : 'text-gray-600 hover:bg-gray-50' }} group transition-colors">
                <span class="flex items-center gap-3">
                    <i class="fa-solid fa-layer-group icon w-5 text-center {{ request()->routeIs('admin.users.*') || request()->routeIs('user.*') ? 'text-blue-500' : 'text-gray-400 group-hover:text-blue-500' }} shrink-0 transition-colors"></i>
                    <span class="label-text text-sm">Manajemen Pengguna</span>
                </span>
                <i :class="open ? 'fa-chevron-up' : 'fa-chevron-down'"
                   class="chevron-icon fa-solid text-[10px] {{ request()->routeIs('admin.users.*') || request()->routeIs('user.*') ? 'text-blue-400' : 'text-gray-300' }} shrink-0"></i>
            </button>
            <div x-show="open"
        ...
        <div x-data="{ open: {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.enroll.*') ? 'true' : 'false' }} }" class="mb-0.5">
            <button @click="open = !open"
                    class="nav-item w-full flex items-center justify-between px-2 py-2.5 rounded-lg {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.enroll.*') ? 'text-blue-600 bg-blue-50/60 font-semibold' : 'text-gray-600 hover:bg-gray-50' }} group transition-colors">
                <span class="flex items-center gap-3">
                    <i class="fa-solid fa-folder-open icon w-5 text-center {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.enroll.*') ? 'text-blue-500' : 'text-gray-400 group-hover:text-blue-500' }} shrink-0 transition-colors"></i>
                    <span class="label-text text-sm">Manajemen Mata Kuliah</span>
                </span>
                <i :class="open ? 'fa-chevron-up' : 'fa-chevron-down'"
                   class="chevron-icon fa-solid text-[10px] {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.enroll.*') ? 'text-blue-400' : 'text-gray-300' }} shrink-0"></i>
            </button>
            <div x-show="open"
                 x-transition:enter="transition ease-out duration-150"
                 x-transition:enter-start="opacity-0"
                ...
                <a href="#" class="block py-2 pr-2 pl-[42px] text-sm text-gray-500 hover:bg-gray-50 hover:text-gray-800 border-l-2 border-transparent transition">Kategori Sesi</a>
            </div>
        </div>
